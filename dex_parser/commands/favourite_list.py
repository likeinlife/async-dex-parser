...
from .title import Commands as TitleCommands
...
class Commands:
	...
	def download(args: argparse.Namespace):
		...
		if 0 <= int(number) <= len(favourite_list) - 1:
			id = favourite_list[number]['id']
			args.id = id
			return TitleCommands.entrypoint(args)
	...
	def delete_item(args: argparse.Namespace):
		...
		write_favourite_list(favourite_list)

		print(f'Delete {deleted["name"]} with id {deleted["id"]} to favourite list')
	...
	def update_item(args: argparse.Namespace):
		...
		print(f'Update {previous_name} with id {updated["id"]} to {args.name}')


def write_favourite_list(favourite_list: list[FavouriteElement]) -> None:
