...
class ImageDownloader(BaseImageDownloader):
	...
	def run(self) -> None:
		asyncio.run(self._download_image_list())

	def _get_path(self, directory: Path | None = None, folder_name: str | None = None) -> Path:
	...
	@staticmethod
	async def _get_image(session: httpx.AsyncClient, image_url: str):
	...
	@staticmethod
	async def _save_image(content: bytes, path_to_file: Path):
	...
	async def _download_image(self, semaphore: asyncio.Semaphore, image_url: str, page_number: int):
		...
		async with semaphore:
			...
			if self._is_override(path_to_file):
				async with httpx.AsyncClient(
					headers=headers.get_image_headers,
					verify=False,
					timeout=config.TIMEOUT,
				) as session:
					content = await self._get_image(session, image_url)
				await self._save_image(content, path_to_file)

	async def _download_image_list(self) -> None:
		...
		tasks = [
			asyncio.create_task(self._download_image(semaphore, image_url, number))
